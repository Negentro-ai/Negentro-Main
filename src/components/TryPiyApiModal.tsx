...
export const TryPiyApiModal: React.FC<TryPiyApiModalProps> = ({ isOpen, onClose }) => {
  const [activeTab, setActiveTab] = useState<'search' | 'store' | 'bitemporal'>('search')
  const [query, setQuery] = useState('user prefer typescript and functional components')
      ...
      <div className="w-full max-w-3xl rounded-3xl bg-white border border-neutral-200 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-200 bg-neutral-50/80">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-semibold text-neutral-900">PiyAPI Live Console</h3>
              <p className="text-[11px] text-neutral-500 font-mono">cluster: us-east-1 • latency: 11ms</p>
            </div>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="p-3.5 rounded-xl bg-neutral-50 border border-neutral-200 space-y-2">
                <div className="flex justify-between text-xs font-medium text-neutral-700">
                  ...
                  <span className="font-mono text-neutral-900 font-semibold">{alpha}</span>
                </div>
                <input
                  type="range"
                  min="0"
...
{JSON.stringify(
  {
    ...
    matches: [
      {
        id: 'mem_8912',
        content: 'User prefers dark mode and concise TypeScript responses with functional components.',
        scores: {
          vector: 0.94,
          bm25: 0.88,
          hybrid: Number((alpha * 0.94 + (1 - alpha) * 0.88).toFixed(3)),
        },
        bitemporal: {
            ...
            <div className="space-y-3 text-xs text-neutral-700">
              <div className="p-4 rounded-xl bg-neutral-50 border border-neutral-200 space-y-2">
                ...
                <p className="text-neutral-600 leading-relaxed text-xs">
                  Query the memory state as it was on any historical timestamp without destructive overwrites.
                </p>
                <div className="mt-3 p-3 rounded-lg bg-neutral-950 text-emerald-400 font-mono text-[11px]">
                  <code>
                    client.query_at(query="system auth config", as_of="2026-08-01T00:00:00Z")
                  </code>
                </div>
              </div>
            </div>
